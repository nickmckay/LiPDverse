---
title: "MS15VIBC"
output: 
  flexdashboard::flex_dashboard:
    theme: bootstrap
    source_code: embed
    orientation: rows
---

```{r setup, include = FALSE}
library(flexdashboard)
library(maptools)
library(tidyverse)
library(purrr)
library(leaflet)
library(plotly)
library(lipdR)
library(dygraphs)
library(geoChronR)

#read functions
source("../../functions.R")


load("../../temp.Rdata")

#remove columns we don't want to plot
varNames <- sapply(TS, "[[","paleoData_variableName")

# good <- which(!(varNames %in% c("year","depth","age")))
# TS <- TS[good]


#All datasets
dsn <- sapply(TS,"[[","dataSetName")
ui <- which(!duplicated(dsn))
udsn <- dsn[ui]
lat <- as.numeric(sapply(TS,"[[","geo_latitude")[ui])
lon <- as.numeric(sapply(TS,"[[","geo_longitude")[ui])
#elev <- sapply(TS,"[[","geo_elevation")[ui]
archiveType <- sapply(TS,"[[","archiveType")[ui]
link <- paste0(udsn,".html") %>% 
  str_replace_all("'","_")

#Organize metadata for map
map.meta <- data.frame(dataSetName = udsn, #datasetname
                       lat = lat,#lat
                       lon = lon,#lon
                      # elev = elev,#elevation
                       archiveType = factor(archiveType),#archiveType
                       link = link)#Link


#set index number

i = 407
thisTS <- TS[which(udsn[i] == dsn)]
```
#MS15VIBC

Metadata {.sidebar}
-------------------------------------
[Download LiPD file](MS15VIBC.lpd)
            

[Edit LiPD file](http://lipd.net/playground?source=http://lipdverse.org/iso2kv0_11_0/MS15VIBC.lpd)
            

[Download data only (csv)](MS15VIBC.csv)
            

[Report an issue (include dataset name)](https://github.com/nickmckay/LiPDverse/issues)

<details open style="margin-left: 0px">
<summary>root</summary>
<p style="margin-left: 0px"><strong>archiveType: </strong>marine sediment
<p style="margin-left: 0px"><strong>lipdVersion: </strong>1.3
</details>
<details>
<summary>pub</summary>
<details style="margin-left: 10px">
<summary>pub1</summary>
<p style="margin-left: 10px"><strong>author: </strong>Voigt, I
<p style="margin-left: 10px"><strong>pubYear: </strong>NA
<p style="margin-left: 10px"><strong>DOI: </strong>10.1002/2014PA002677
</details>
<details style="margin-left: 10px">
<summary>pub2</summary>
<p style="margin-left: 10px"><strong>author: </strong>Chiessi, C.M.
<p style="margin-left: 10px"><strong>pubYear: </strong>2014
</details>
</details>
<details open style="margin-left: 0px">
<summary>geo</summary>
<p style="margin-left: 0px"><strong>latitude: </strong>-32.5052
<p style="margin-left: 0px"><strong>longitude: </strong>-50.2427
<p style="margin-left: 0px"><strong>elevation: </strong>-657
<p style="margin-left: 0px"><strong>siteName: </strong>Brazil Current
</details>
<details open>
<summary>PaleoData columns</summary>
<details style="margin-left: 10px">
<summary>year (AD)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>MAT2ac14034e6
<p style="margin-left: 10px"><strong>variableName: </strong>year
<p style="margin-left: 10px"><strong>units: </strong>AD
<p style="margin-left: 10px"><strong>description: </strong>Year AD
</details>
<details style="margin-left: 10px">
<summary>depth (cm)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>LPD08287935w
<p style="margin-left: 10px"><strong>variableName: </strong>depth
<p style="margin-left: 10px"><strong>units: </strong>cm
<p style="margin-left: 10px"><strong>description: </strong>composite depth
</details>
<details style="margin-left: 10px">
<summary>depth (cm)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>LPD08287935
<p style="margin-left: 10px"><strong>variableName: </strong>depth
<p style="margin-left: 10px"><strong>units: </strong>cm
<p style="margin-left: 10px"><strong>description: </strong>depth
</details>
<details style="margin-left: 10px">
<summary>d18O (permil)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>MS14CCBC01b
<p style="margin-left: 10px"><strong>variableName: </strong>d18O
<p style="margin-left: 10px"><strong>units: </strong>permil
<p style="margin-left: 10px"><strong>description: </strong>inferred seawater
<details>
<summary>interpretation</summary>
<details style="margin-left: 20px">
<summary>1</summary>
<p style="margin-left: 20px"><strong>basis: </strong>NaN
<p style="margin-left: 20px"><strong>interpDirection: </strong>positive
<p style="margin-left: 20px"><strong>scope: </strong>climate
<p style="margin-left: 20px"><strong>seasonality: </strong>NaN
<p style="margin-left: 20px"><strong>variable: </strong>salinity
<p style="margin-left: 20px"><strong>variableDetail: </strong>surface
<p style="margin-left: 20px"><strong>direction: </strong>positive
</details>
<details style="margin-left: 20px">
<summary>2</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>positive
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>sea surface
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>linear
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>Salinity
</details>
<details style="margin-left: 20px">
<summary>3</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
<details style="margin-left: 20px">
<summary>4</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
</details>
</details>
<details style="margin-left: 10px">
<summary>d18O (permil)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>MS15VIBC01
<p style="margin-left: 10px"><strong>variableName: </strong>d18O
<p style="margin-left: 10px"><strong>units: </strong>permil
<p style="margin-left: 10px"><strong>description: </strong>carbonate
<details>
<summary>interpretation</summary>
<details style="margin-left: 20px">
<summary>1</summary>
<p style="margin-left: 20px"><strong>basis: </strong>NaN
<p style="margin-left: 20px"><strong>interpDirection: </strong>decrease/increase
<p style="margin-left: 20px"><strong>scope: </strong>climate
<p style="margin-left: 20px"><strong>seasonality: </strong>annual mean
<p style="margin-left: 20px"><strong>variable: </strong>temperature/d18Osw
<p style="margin-left: 20px"><strong>variableDetail: </strong>subsurface (300-400m)
</details>
<details style="margin-left: 20px">
<summary>2</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Urey, H. C. 1948. Oxygen isotopes in nature and in the laboratory. Science, 108:489?496.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>negative
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>seawater
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>nonlinear
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>T_water
<p style="margin-left: 20px"><strong>rank: </strong>NA
</details>
<details style="margin-left: 20px">
<summary>3</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Urey, H. C. 1948. Oxygen isotopes in nature and in the laboratory. Science, 108:489?496.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>decrease
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>calcite, subsurface (300-400m)
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>non-linear
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>seawater_isotope
<p style="margin-left: 20px"><strong>rank: </strong>NA
</details>
<details style="margin-left: 20px">
<summary>4</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Spero, H.J., Bijma, J., Lea, D.W., Bemis, B.E., 1997. Effect of seawater carbonate concentration on foraminiferal carbon and oxygen isotopes. Nature 390, 497-500.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>negative
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>calcite, subsurface (300-400m)
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>linear
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>Carbonate_ion_concentration
<p style="margin-left: 20px"><strong>rank: </strong>NA
</details>
</details>
</details>
<details style="margin-left: 10px">
<summary>d18O (permil)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>MS14CCBC02
<p style="margin-left: 10px"><strong>variableName: </strong>d18O
<p style="margin-left: 10px"><strong>units: </strong>permil
<p style="margin-left: 10px"><strong>description: </strong>carbonate
<details>
<summary>interpretation</summary>
<details style="margin-left: 20px">
<summary>1</summary>
<p style="margin-left: 20px"><strong>scope: </strong>climate
</details>
<details style="margin-left: 20px">
<summary>2</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Urey, H. C. 1948. Oxygen isotopes in nature and in the laboratory. Science, 108:489?496.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>negative
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>calcite, thermocline
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>nonlinear
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>T_water
</details>
<details style="margin-left: 20px">
<summary>3</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Urey, H. C. 1948. Oxygen isotopes in nature and in the laboratory. Science, 108:489?496.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>negative
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>calcite, thermocline
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>non-linear
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>seawater_isotope
</details>
<details style="margin-left: 20px">
<summary>4</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Spero, H.J., Bijma, J., Lea, D.W., Bemis, B.E., 1997. Effect of seawater carbonate concentration on foraminiferal carbon and oxygen isotopes. Nature 390, 497-500.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>negative
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>calcite, thermocline
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>linear
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>Carbonate_ion_concentration
</details>
</details>
</details>
<details style="margin-left: 10px">
<summary>core ()</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>LPD08287935h
<p style="margin-left: 10px"><strong>variableName: </strong>core
<details>
<summary>interpretation</summary>
<details style="margin-left: 20px">
<summary>1</summary>
<p style="margin-left: 20px"><strong>scope: </strong>climate
</details>
<details style="margin-left: 20px">
<summary>2</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
<details style="margin-left: 20px">
<summary>3</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
<details style="margin-left: 20px">
<summary>4</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
</details>
</details>
<details style="margin-left: 10px">
<summary>d18O (permil)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>MS14CCBC01a
<p style="margin-left: 10px"><strong>variableName: </strong>d18O
<p style="margin-left: 10px"><strong>units: </strong>permil
<p style="margin-left: 10px"><strong>description: </strong>carbonate
<details>
<summary>interpretation</summary>
<details style="margin-left: 20px">
<summary>1</summary>
<p style="margin-left: 20px"><strong>scope: </strong>climate
</details>
<details style="margin-left: 20px">
<summary>2</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Urey, H. C. 1948. Oxygen isotopes in nature and in the laboratory. Science, 108:489?496.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>negative
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>calcite, surface
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>nonlinear
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>T_water
</details>
<details style="margin-left: 20px">
<summary>3</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Urey, H. C. 1948. Oxygen isotopes in nature and in the laboratory. Science, 108:489?496.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>negative
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>calcite, surface
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>non-linear
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>seawater_isotope
</details>
<details style="margin-left: 20px">
<summary>4</summary>
<p style="margin-left: 20px"><strong>basis: </strong>Spero, H.J., Bijma, J., Lea, D.W., Bemis, B.E., 1997. Effect of seawater carbonate concentration on foraminiferal carbon and oxygen isotopes. Nature 390, 497-500.
<p style="margin-left: 20px"><strong>coefficient: </strong>NaN
<p style="margin-left: 20px"><strong>direction: </strong>negative
<p style="margin-left: 20px"><strong>equilibriumEvidence: </strong>NaN
<p style="margin-left: 20px"><strong>fraction: </strong>NaN
<p style="margin-left: 20px"><strong>inferredMaterial: </strong>calcite, surface
<p style="margin-left: 20px"><strong>integrationTime: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeBasis: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertainty: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUncertaintyType: </strong>NaN
<p style="margin-left: 20px"><strong>integrationTimeUnits: </strong>NaN
<p style="margin-left: 20px"><strong>mathematicalRelation: </strong>linear
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
<p style="margin-left: 20px"><strong>seasonality: </strong>not applicable (always)
<p style="margin-left: 20px"><strong>variable: </strong>Carbonate_ion_concentration
</details>
</details>
</details>
<details style="margin-left: 10px">
<summary>Mg_Ca (mmol/mol)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>LPD08287935c
<p style="margin-left: 10px"><strong>variableName: </strong>Mg_Ca
<p style="margin-left: 10px"><strong>units: </strong>mmol/mol
<p style="margin-left: 10px"><strong>description: </strong>Mg/Ca of foraminifera calcite
<details>
<summary>interpretation</summary>
<details style="margin-left: 20px">
<summary>1</summary>
<p style="margin-left: 20px"><strong>scope: </strong>climate
</details>
<details style="margin-left: 20px">
<summary>2</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
<details style="margin-left: 20px">
<summary>3</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
<details style="margin-left: 20px">
<summary>4</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
</details>
</details>
<details style="margin-left: 10px">
<summary>temperature (deg C)</summary>
<p style="margin-left: 10px"><strong>TSid: </strong>LPD08287935r
<p style="margin-left: 10px"><strong>variableName: </strong>temperature
<p style="margin-left: 10px"><strong>units: </strong>deg C
<p style="margin-left: 10px"><strong>description: </strong>Mg/Ca derived annual mean SST
<details>
<summary>interpretation</summary>
<details style="margin-left: 20px">
<summary>1</summary>
<p style="margin-left: 20px"><strong>scope: </strong>climate
</details>
<details style="margin-left: 20px">
<summary>2</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
<details style="margin-left: 20px">
<summary>3</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
<details style="margin-left: 20px">
<summary>4</summary>
<p style="margin-left: 20px"><strong>coefficient: </strong>NA
<p style="margin-left: 20px"><strong>fraction: </strong>NA
<p style="margin-left: 20px"><strong>rank: </strong>NA
<p style="margin-left: 20px"><strong>scope: </strong>isotope
</details>
</details>
</details>

Row {.tabset .tabset-fade}
-----------------------------------------------------------------------

### Sitemap

```{r}
  map.meta.split <- split(map.meta, map.meta$archiveType)


factpal <- colorFactor("Paired",map.meta$archiveType)
buff <- 15
l <- leaflet() %>% 
  addTiles() %>% 
  fitBounds(map.meta$lon[i]-buff,map.meta$lat[i]-buff,map.meta$lon[i]+buff,map.meta$lat[i]+buff)
  

  
  
names(map.meta.split) %>%
  purrr::walk( function(df) {
    l <<- l %>%
      addMarkers(data=map.meta.split[[df]],
                 lng=~lon, lat=~lat,
                 label=~as.character(archiveType),
                 popup=~paste(str_c('Dataset: <a href="',link,'">',dataSetName,'</a>'),
                                 # str_c("Elevation: ",elev),
                                  str_c("Archive Type: ",archiveType),
                                  sep = "<br>"),
                 group = df,
                 clusterOptions = markerClusterOptions(removeOutsideVisibleBounds = F),
                 labelOptions = labelOptions(noHide = F,
                                             direction = 'auto'))
  })

l %>%
  addLayersControl(position = "bottomleft",
    overlayGroups = names(map.meta.split),
    options = layersControlOptions(collapsed = FALSE,
                                   opacity = 0.8)
  )
```

### Search the LiPDverse (Beta! doesn't work well yet)
```{r}
#Add google search bar
htmltools::includeHTML("../../googleSearchChunk.html")
```


Row {.tabset .tabset-fade}
-----------------------------------------------------------------------

### d18O (permil)
```{r}
plotCol(thisTS,ind = 6)
```

### d18O (permil)
```{r}
plotCol(thisTS,ind = 9)
```

### d18O (permil)
```{r}
plotCol(thisTS,ind = 1)
```

### core ()
```{r}
plotCol(thisTS,ind = 4)
```

### d18O (permil)
```{r}
plotCol(thisTS,ind = 5)
```

### Mg_Ca (mmol/mol)
```{r}
plotCol(thisTS,ind = 7)
```

### temperature (deg C)
```{r}
plotCol(thisTS,ind = 8)
```
